...
def count_decimal_digits(x: Decimal) -> Tuple[int, int]:
    ...
    precision = len(s_digits.lstrip('0')) or 1
...
def validate_value(field_schema: Any, raw_value: str, field_name: str) -> List[Tuple[str, str]]:
    ...
    if isinstance(effective_schema, dict):
        ...
        if logical_type in ("timestamp-millis", "timestamp-micros"):
            ...
            if parse_timestamp(raw_value, unit) is None:
                errors.append(("TYPE_TIMESTAMP", f"Invalid timestamp for '{field_name}'"))
            return errors
        ...
        if logical_type == "decimal":
            ...
            if precision is None or scale is None:
                errors.append(("SCHEMA_DECIMAL", f"Decimal '{field_name}' missing precision/scale"))
                return errors
            ...
            if scl > scale:
                errors.append(("DECIMAL_SCALE", f"Scale {scl} exceeds schema scale {scale}"))
            if prec > precision:
                errors.append(("DECIMAL_PRECISION", f"Precision {prec} exceeds schema precision {precision}"))
            return errors
        ...
        if base_type == "array":
            ...
            for idx, item in enumerate(arr):
                sub_errors = validate_value(items_schema, str(item), f"{field_name}[{idx}]")
        ...
        if base_type == "record":
            ...
            for key in obj.keys():
                if key not in schema_fields:
                    errors.append(("EXTRA_NESTED_FIELD", f"Extra nested field '{field_name}.{key}'"))
            return errors
    ...
    if effective_schema == "boolean":
        if parse_bool(raw_value) is None:
            errors.append(("TYPE_BOOLEAN", f"Invalid boolean for '{field_name}'"))
        return errors
    ...
    if effective_schema == "int":
        ...
        if i is None:
            ...
            if i < INT32_MIN or i > INT32_MAX:
                errors.append(("RANGE_INT", f"Value {i} out of 32-bit int range"))
        return errors
    ...
    if effective_schema in ("long", "bigint"):
        ...
        if i is None:
            ...
            if i < INT64_MIN or i > INT64_MAX:
                errors.append(("RANGE_LONG", f"Value {i} out of 64-bit long range"))
        return errors

    if effective_schema == "float":
        if parse_float(raw_value) is None:
...
def validate_csv_against_avro(schema_path: str, csv_path: str,
    ...
    if bad_rows:
        ...
        with open(bad_records_out, "w", encoding="utf-8", newline="") as bf:
            writer = csv.DictWriter(bf, fieldnames=out_fields, delimiter=";")
            writer.writeheader()
            for r in bad_rows:
                writer.writerow(r)

    with open(error_log_out, "w", encoding="utf-8", newline="") as ef:
        writer = csv.DictWriter(ef, fieldnames=["row_number", "column", "error_code", "message", "value"], delimiter=";")
        writer.writeheader()
        for e in error_log:
            writer.writerow(e)
...
if __name__ == "__main__":
    validate_csv_against_avro(
        schema_path="SCHEMA5.avsc",
        csv_path="SCHEMA5_output.csv",
